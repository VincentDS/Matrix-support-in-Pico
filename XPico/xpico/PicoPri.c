...
#define MAT_TEXT "<matrix>"
#define MTX_TEXT "<matrication>"
...
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ MAt(_NIL_TYPE_)
{ _EXP_TYPE_ nbr, mat, voi, dim, tab, exp;
    _UNS_TYPE_ idx, siz;
    _stk_claim_();
    _stk_pop_EXP_(voi);
    _stk_pop_EXP_(nbr);
    ...
    if (idx < siz)
    { _stk_push_EXP_(_ag_succ_NBR_(nbr));
      exp = _ag_get_TAB_EXP_(tab, idx+1);
      _stk_push_EXP_(exp);
      _stk_push_CNT_(EXP);
      _print_(", "); }
    else
