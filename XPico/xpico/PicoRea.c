...
/*  FCT                                                                   */
/*     expr-stack: [... ... ... ... ... ...] -> [... ... ... ... ... ...] */
/*     cont-stack: [... ... ... ... CNT FCT] -> [... ... ... CNT FCt REF] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ FCT(_NIL_TYPE_)
 { _stk_claim_();
   _stk_poke_CNT_(FCt);
...
/*     cont-stack: [... ... ... ... CNT IDX] -> [... ... ... CNT IDx EXP] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ IDX(_NIL_TYPE_)
{ _stk_claim_(); //we put more continuations on the cont-stack than we took away
  _stk_push_EXP_(_ONE_); //for the index in the table
  _stk_poke_CNT_(IDx);
  _stk_push_CNT_(EXP); } //calculates the current index-expression
...
{ _EXP_TYPE_ exp, nbr, tab;
  ...
  _stk_claim_();
  _stk_pop_EXP_(exp);
  _stk_peek_EXP_(nbr); //take last number on stack to know which size the tab must be
  _stk_poke_EXP_(exp);
  switch (current_token) //is the next token a comma or right bracket?
    { case _RBR_TOKEN_: //right-bracket: the end of the index-table => put table on expr-stack
        READ_TOKEN();
        ...
        do
        { _stk_pop_EXP_(exp);
            _ag_set_TAB_EXP_(tab, idx, exp); } //filling tab with arguments on expr-stack
        while (--idx);
        _stk_push_EXP_(tab);
        _stk_zap_CNT_();
        break;
      case _COM_TOKEN_:
        READ_TOKEN();
        nbr = _ag_succ_NBR_(nbr);
        _stk_push_EXP_(nbr); //tab becomes larger, put next index on expr-stack
        _stk_push_CNT_(EXP);
        break;
      default:
        _scan_error_(_RBR_ERROR_); }}
...
/*  INV                                                                   */
/*     expr-stack: [... ... ... ... ... ...] -> [... ... ... ... ... ...] */
/*     cont-stack: [... ... ... ... CNT INV] -> [... ... ... CNT INv CMP] */
/*------------------------------------------------------------------------*/
static _NIL_TYPE_ INV(_NIL_TYPE_)
 { _stk_claim_();
   _stk_poke_CNT_(INv);
...
/*                                                                        */
/*     expr-stack: [... ... ... ... NAM EXP] -> [... ... ... ... ... MTX] */
/*     cont-stack: [... ... ... ... CNT TBL] -> [... ... ... ... ... CNT] */
/*------------------------------------------------------------------------*/
//TBL can also make a matrix now. If the exp is a table of size 1, it's a TBL.
//Otherwise we have a matrix.
static _NIL_TYPE_ TBL(_NIL_TYPE_)
 { _EXP_TYPE_ dim, nam, tbl, idx, mtx;
   _UNS_TYPE_ siz;
   _mem_claim_(); //mem_claim so we have surely enough memory to store a tabulation or matrix
   _stk_pop_EXP_(dim); //the dimension-table
   siz = _ag_get_TAB_SIZ_(dim);
   _stk_peek_EXP_(nam); //name of the table
   _stk_zap_CNT_(); //remove TBL-continuation from stack
   if (siz == 1) {
       tbl = _ag_make_TBL_();
       ...
       mtx = _ag_make_MTX_();
       _ag_set_MTX_NAM_(mtx, nam);
       _ag_set_MTX_DIM_(mtx, dim);
       _stk_poke_EXP_(mtx);
   }
 }
