...
static _NIL_TYPE_ DVM(_NIL_TYPE_);
...
static _NIL_TYPE_ DVU(_NIL_TYPE_);
...
{ _EXP_TYPE_ tab, val;
  ...
  _stk_zap_CNT_();
  if (tag == _MAT_TAG_)
  { tab = _ag_get_MAT_DIM_(val);
    _stk_poke_EXP_(_ag_make_NBU_(_ag_get_TAB_SIZ_(tab)));
    return; }
  _error_str_(_ATC_ERROR_, dim_STR); }

/*------------------------------------------------------------------------*/
/*  DVM                                                                   */
/*     expr-stack: [... ... ... ... ... ARG] -> [... ... ... ... ... EVL] */
/*     cont-stack: [... ... ... ... ... DVU] -> [... ... ... ... DVU EVL] */
/*------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------*/
/*  MTU                                                                   */
/*     expr-stack: [... ... ... ... VAL VAL] -> [... ... ... ... ... MAT] */
/*     cont-stack: [... ... ... ... ... MTU] -> [... ... ... ... ... ...] */
/*------------------------------------------------------------------------*/

static _NIL_TYPE_ MTU(_NIL_TYPE_)
{ _EXP_TYPE_ dim, tab, mat;
  _TAG_TYPE_ tag_1, tag_2;
  _UNS_TYPE_ siz_1, siz_2, ctr, tot;
  _mem_claim_();
  _stk_pop_EXP_(tab);
  _stk_peek_EXP_(dim);
  _stk_zap_CNT_();
  tag_1 = _ag_get_TAG_(tab);
  tag_2 = _ag_get_TAG_(dim);
  tot = 1;
  if ((tag_1 == _TAB_TAG_) && (tag_2 == _TAB_TAG_))
    { siz_1 = _ag_get_TAB_SIZ_(tab);
      siz_2 = _ag_get_TAB_SIZ_(dim);
      for (ctr = 1; ctr <= siz_2; ctr++)
         { tot *= _ag_get_NBU_(_ag_get_TAB_EXP_(dim, ctr)); }
      if (tot == siz_1)
        { mat = _ag_make_MAT_();
          _ag_set_MAT_DIM_(mat, dim);
          _ag_set_MAT_TAB_(mat, tab);
          _stk_poke_EXP_(mat);
        } else
       _error_str_(_SIZ_ERROR_, mtx_STR);
    } else
    _error_str_(_NAT_ERROR_, mtx_STR); }
